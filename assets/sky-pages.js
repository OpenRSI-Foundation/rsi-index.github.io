/* ==========================================================================
   rsi-index — "sky-pages": the hero's star-trail sky (assets/startrail3d.js)
   run as ONE fixed, full-viewport field behind a whole page, the way the
   GPT-6 Astra article keeps its particle canvas under the text. Same dome,
   same converge, same drift; what changes:
     · host = [data-sky-page], a position:fixed box the size of the viewport
     · no landscape and no bokeh: a pure sky, so nothing sits at the bottom
       of every screen while the page scrolls
     · the field "disperses" as the hero scrolls away (window.scrollY over
       the hero's height): the arcs shorten, the stars dim and shrink, the
       view eases up and widens. document.documentElement carries
       --sky-dsp (0..1) and --sky-col (the alpha left over the reading
       column) for the stylesheet's safe-area mask.
   Derived from startrail3d.js at 03bca0e; keep the two in step by hand.

   The circumpolar star-trail timelapse as a true
...
(function () {
  ...
  function boot() {
    ...
    var host = document.querySelector('[data-sky-page]');
    if (!host) return;
    ...
    if (window.matchMedia) {
      ...
      var onMq = function () { reduced = !!mq.matches || STATIC; };
    ...
    function bake() {
      ...
      /* ambient: a faint blue breath at the pole, like the reference's #23435F */
      var amb = skx.createRadialGradient(sky.width / 2, sky.height * 0.42, 0, sky.width / 2, sky.height * 0.42, Math.max(sky.width, sky.height) * 0.55);
      amb.addColorStop(0, 'rgba(35, 67, 95, 0.22)');
      amb.addColorStop(1, 'rgba(35, 67, 95, 0)');
      skx.fillStyle = amb;
      skx.fillRect(0, 0, sky.width, sky.height);
    }
    function resize() {
    ...
    /* ---------------- dispersal: the hero scrolls away, the field recedes ----
       target = scrollY over the hero's height (or DISPERSE px), smootherstepped;
       the drawn value follows it with exponential damping (lambda 6). */
    var DISPERSE = 800;
    var heroEl = document.querySelector('[data-sky-hero]');
    var dspT = 0, dsp = 0;
    function sstep(x) { return x <= 0 ? 0 : x >= 1 ? 1 : x * x * x * (x * (x * 6 - 15) + 10); }
    function readScroll() {
      var span = heroEl ? Math.max(240, heroEl.offsetHeight * 0.85) : DISPERSE;
      dspT = sstep(((window.scrollY || window.pageYOffset || 0) - 40) / span);
      if (!active) { dsp = dspT; publish(); }
    }
    function publish() {
      var d = doc2.style;
      d.setProperty('--sky-dsp', dsp.toFixed(3));
      d.setProperty('--sky-col', (1 - 0.78 * dsp).toFixed(3));
      host.setAttribute('data-sky-state', dsp > 0.5 ? 'page' : 'hero');
      window.__skyPage = { dsp: dsp, target: dspT };
    }
    /* the light column starts inside the hero's foot: tell the stylesheet
       how tall the hero really is */
    function measure() {
      if (heroEl) document.documentElement.style.setProperty('--hero-h', heroEl.offsetHeight + 'px');
    }
    ...
    /* the field is fixed to the viewport, so the hero scrolling away does not hide it:
       'rsi-sky-visibility' is for the hero's own inline sky, not for this one */
    var t0 = -1;
    ...
    function frame(now) {
      ...
      /* the field recedes with the hero: damped follow of the scroll target */
      dsp += (dspT - dsp) * (1 - Math.exp(-6 * dt));
      if (Math.abs(dspT - dsp) < 0.0015) dsp = dspT;
      publish();
      var EXP_NOW = EXPOSE * (1 - 0.52 * dsp);    /* arcs shorten            */
      var BM = 1 - 0.56 * dsp;                    /* brightness              */
      var SM = 1 - 0.30 * dsp;                    /* head size, trail width  */
      var PITCH_D = PITCH0 + 0.16 * dsp;          /* the view eases upward   */
      var FOV_D = FOV0 * (1 + 0.14 * dsp);        /* ...and widens           */

      if (resetting) {
        var kk = 1 - Math.pow(0.002, dt);
        var dy2 = -yaw; dy2 -= TAU * Math.round(dy2 / TAU);
        yaw += dy2 * kk;
        pitch += (PITCH_D - pitch) * kk;
        fov += (FOV_D - fov) * kk;
        if (Math.abs(dy2) < 0.003 && Math.abs(pitch - PITCH_D) < 0.003 &&
            Math.abs(fov - FOV_D) < 0.003) resetting = false;
      } else if (!dragging) {
      ...
      if (!reduced && !dragging && !resetting &&
        ...
        var pT = PITCH_D + 0.018 * Math.sin(idleT * 0.12 + 1.1);
        var fT = FOV_D + 0.022 * Math.sin(idleT * 0.07 + 1.7);
      ...
      if (!reduced) {
        if (!BGMODE && t < GATHER) {
          ...
          if (phi - expA >= EXP_NOW) expA = phi - EXP_NOW;
      ...
      for (var i = 0; i < NT; i++) {
        ...
        for (var epi = 0; epi < 2; epi++) {
          ...
          var trailA = sB[i] * ext * epAl * 0.92 * BM;   /* uniform, video-metered  */
          ...
          if (b2 > 0.72) {
            ...
            if (qh >= 1) {
              ...
              ctx.lineWidth = sLw[i] * 2.8 * dpr * SM;
              ctx.beginPath();
              var pen0 = false;
              for (k2 = 0; k2 <= ns; k2++) {
          ...
          for (var ch0 = 0; ch0 < ns; ch0 += 2) {
            ...
            ctx.lineWidth = sLw[i] * wm2 * dpr * SM;
            ctx.beginPath();
            var pen1 = false;
      ...
      ctx.globalCompositeOperation = 'lighter';
      var landA = g;
      for (i = 0; i < 0; i++) {   /* page mode: no bokeh */
      ...
      var span = fv * (w / h) + 1.2;
      var M = 160;
      for (var rg = 0; rg < 0; rg++) {   /* page mode: no landscape */
        var isNear = rg === 1;
      ...
      ctx.globalAlpha = 1;
      if (FIXT >= 0) { dsp = dspT; publish(); return; }   /* a frozen clock renders one frame */
      schedule();
    }
    schedule();
  }

  /* the hero title, one letter at a time but never a broken word: each word
     is a nowrap span of letter spans; --i orders the entrance from the left */
  function splitTitle() {
    var els = document.querySelectorAll('.sky-hero [data-split]');
    var i = 0;   /* one running index across every split span, so a title in two parts enters left to right */
  ...
  function start() { splitTitle(); boot(); }
